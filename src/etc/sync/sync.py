...
for i in ldap_search(os.environ['LDAP_BASE_DN'],
#	searchFilter = "({})".format(os.environ['LDAP_FILTER']),
	searchFilter = "({})".format("objectClass=groupOfMembers"),
	retrieveAttributes = []): 
...
class GROUP(object):
    ...
    def json(self):
        return { "name" : self.name, 'members': self.members, 'current': self.current() }
